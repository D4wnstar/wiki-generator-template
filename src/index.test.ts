...
import { slugPath, getPathCombinations, deletePathAndDescendants } from '$lib/notes'

describe('slugifyPath function', () => {
	it('should return a slug for a single word', () => {
		expect(slugPath('Hello')).toBe('hello')
	})

	it('should return a slug for a sentence', () => {
		expect(slugPath('Hello World')).toBe('hello-world')
	})

	it('should return a slug for a path with spaces', () => {
		expect(slugPath('Hello World/This is a test')).toBe('hello-world/this-is-a-test')
	})

	it('should remove special characters', () => {
		expect(slugPath("Don't use special characters!")).toBe('dont-use-special-characters')
	})

	it('should return a slug for a path with special characters', () => {
		expect(slugPath("Hello/Don't use special characters!")).toBe(
			'hello/dont-use-special-characters'
		)
	})

	it('should return a slug for a url', () => {
		expect(slugPath("some path/with spaces/don't/Break")).toBe('some-path/with-spaces/dont/break')
	})
})

describe('getPathCombinations function', () => {
	it('should return all combinations for a given path', () => {
		expect(getPathCombinations('path/to/folder')).toEqual(
			new Set(['path', 'path/to', 'path/to/folder'])
		)
	})
...
describe('deletePathAndDescendants function', () => {
	...
	it('does not delete unrelated paths', () => {
		const paths = new Set(['path/to', 'path/to/a', 'path/other', 'path/other/d'])
